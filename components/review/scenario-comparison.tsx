...
/**
 * Optional illustrative sensitivity comparison for the Review value
...
const cardTone: Record<string, string> = {
  LOWER_REALIZATION: "border-[#d7c9ae] bg-white/70",
  BASE_REALIZATION: "border-[#d89b2b] bg-[#fff6df]",
  HIGHER_REALIZATION: "border-[#c6d8cb] bg-[#f0f6f1]",
};
  ...
  return (
    <details className="rounded-3xl border border-[#e8dcc6] bg-[#fffaf0] p-8">
      <summary className="cursor-pointer text-2xl font-bold text-[#0b1d33]">
        Illustrative Sensitivity <span className="text-base font-semibold text-[#627085]">(optional)</span>
      </summary>
      <p className="mt-3 max-w-3xl text-[#627085]">
        A display-only sensitivity band around the identified opportunity. The Base Realization case matches the deterministic analysis exactly. Sensitivity factors scale only the canonical benefit outputs; investment costs, module inputs, overlap dispositions, and informational capital are unchanged.
      </p>
      <p className="mt-3 max-w-3xl text-sm font-semibold text-[#35465c]">{comparison.disclaimer}</p>
        ...
        {comparison.cases.map((scenario) => (
          <article key={scenario.key} className={`rounded-2xl border p-5 ${cardTone[scenario.key] ?? "border-[#d7c9ae] bg-white/70"}`}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-sm font-semibold text-[#627085]">{(scenario.factor * 100).toLocaleString(undefined, { maximumFractionDigits: 0 })}%</span>
            </div>
